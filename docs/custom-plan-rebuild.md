...
只修改这条托盘创建路径，不能由此证明不存在其他独立创建者。软件更新后应重新检查版本及内容，不能强行套用旧偏移。

## 本机应用记录（2026-09-06）

已在 Legion Y7000P IRH8 / Legion Zone 2.0.28.8182 应用。首次尝试备份完成后遇到托盘退出时序问题；随后补充持有进程句柄并等待退出（最多 10 秒），保留首次备份，以独立备份目录重试成功。

磁盘 DLL SHA256 与表中的补丁后哈希一致，托盘重启后实际加载目标 DLL。初始化后等待 12 秒，自定义 GUID 未出现，原非联想活动计划保持不变。Authenticode 检查为预期的 HashMismatch，没有更改签名验证策略。

未验证游戏启动、系统重启、所有性能模式切换或长期稳定性。真实恢复流程未执行；恢复、加载失败回滚及等待退出超时使用隔离测试验证。本机原 DLL、计划备份、进程信息和日志均未提交到公开仓库。
